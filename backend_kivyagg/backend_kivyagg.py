...
class RendererKivyAgg(RendererBase):
    """
...
class GraphicsContextKivyAgg(GraphicsContextBase):
    """
...
class FigureCanvasKivyAgg(FigureCanvasAgg):
    ...
    def draw(self):
        ...
        texture = Texture.create(size=(500, 400))
        texture.blit_buffer(buf_rgba, colorfmt='rgba', bufferfmt='ubyte')
        texture.flip_vertical()
        with self.widget.canvas:
            Rectangle(texture=texture, pos=self.widget.pos, size=(500, 400))
    ...
    def print_foo(self, filename, *args, **kwargs):
        ...
        """
        im = Image.frombuffer('RGBA', (500,400), self.get_renderer().buffer_rgba(), "raw", "RGBA", 0, 1)
        im.save(filename)
